...
int howManyBits(int x) {
  int hb16,hb8,hb4,hb2,hb1,hb0,isZero,sum,sign;
  //首先判断是不是0的情形
  isZero = x^0;//是0返回0
  sum = 1; //提前加上符号位的1
  //对于负数我们需要先取反
  sign = x>>31; //注意这里是符号右移int类型默认是符号右移，负数就是全1,正数就是全0
  x = (sign & ~x) | (~sign & x);
  //开始二分查找
...
unsigned floatPower2(int x) {
  //因为表示2^x所以尾数位只能为0,NAN和0相近的数其实是不用考虑的因为尾数不为0
